...
class Securitee_mcp 
{
	...
	public function __construct()
	{
		...
		$this->EE->load->vars(
			array(
				...
				'theme_folder_url' => $this->EE->config->config['theme_folder_url']
			)
		);	
		...
		if( $this->settings['disable_accordions'] == '1' )
		{
			$this->EE->cp->add_to_foot('<link type="text/css" rel="stylesheet" href="'.m62_theme_url().'securitee/css/accordion.css" />');
		}

		//now the theme goodies
		$this->EE->cp->add_to_foot('<link type="text/css" rel="stylesheet" href="'.m62_theme_url().'securitee/css/chosen.css" />');
		$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.m62_theme_url().'securitee/js/chosen.jquery.js"></script>');
		$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.m62_theme_url().'securitee/js/securitee.js"></script>');
		$this->EE->cp->add_to_foot('<link type="text/css" rel="stylesheet" href="'.m62_theme_url().'securitee/css/securitee.css" />');	
	}
	...
	public function settings()
	{
		...
		{	
			if($this->EE->securitee_lib->update_settings($_POST))
			{	
				...
				$this->EE->functions->redirect($this->url_base.'settings&section='.$this->EE->input->get_post('section'));		
				exit;			
			}
			else
			{
		...
		$vars['menu_data'] = $this->EE->securitee_lib->get_settings_view_menu();
